...
try {
    if ($action == "post") {
        ...
        if ($message == "" && $name == "" && $caption == "" && $link == "" && $description == "" && $picture == "") {
        	...
        	$actions = ""; //json_encode(array('name' => 'نشر الكترونى ', 'link' => 'http://78.46.224.126/sites/facebooky'));
			if($link != "")
			{
				$actions = "";
			}
            ...
            if ($num_rows > 0) {
                ...
                while ($r = mysql_fetch_assoc($allusers)) {
                    ...
                    if ($a == "1") {
                        ...
                        $pagemessages = LoadPhp("http://webservices.egtema3y.com/postfacefeed.php?type=accounts&accesstoken=" . $r["accesstoken"] . "&message=" . $message_enc . "&link=" . $link_enc);
