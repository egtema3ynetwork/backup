...
define('GOURL', 'http://chat.egtema3y.com/');
...
<html lang="en" >
	...
	<body>
		...
		$loginUrl = $facebook -> getLoginUrl($params = array('scope' => APP_SCOPE, 'redirect_uri' => 'http://78.46.224.121/sites/v2/webservices/accesstoken.register.chat.php'));
		...
		if (isset($_GET['code'])) {
			try {
				...
				$accesstoken = spy($accesstoken);
				
				$appid = $facebook -> getAppId();

				$oldusers = runQuery("select `faceid` from `mobile_chat`.`" . TABLE_NAME . "` where `faceid`='$faceid' ");

				
				if ($oldusers->count > 0) {
					

					$sql = "update  `mobile_chat`.`" . TABLE_NAME . "` set `accesstoken` = '{$accesstoken}'  , `time`=CURRENT_TIMESTAMP where `faceid` = '{$faceid}' ";
					execQuery($sql);
				...
				$sql = "INSERT INTO `mobile_chat`.`" . TABLE_NAME . "`(`id`, `name`, `email`, `faceid`,  `accesstoken`, `gender`,`location`,`birthday`) VALUES";
				$sql .= "(null,'{$name}','{$email}','{$faceid}','{$accesstoken}','{$gender}','{$location}','{$birthday}')";
				execQuery($sql);
